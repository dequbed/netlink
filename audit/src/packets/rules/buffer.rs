...
use constants::*;
use {Error, Field, Parseable, Result, Rule, RuleField, RuleFieldFlags, RuleMask};
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<Rule> for RuleBuffer<&'a T> {
    fn parse(&self) -> Result<Rule> {
        use self::RuleField::*;
