...
use {Emitable, Error, Parseable, Result};
...
impl<T: AsRef<[u8]>> Parseable<Status> for T {
    fn parse(&self) -> Result<Status> {
        Status::from_bytes(self.as_ref())
    }
}
